...
export const RecommendationsPage: React.FC<RecommendationsPageProps> = ({
  assessment,
  currentUser, // Anda juga bisa menghapus currentUser jika memang tidak dipakai lagi di mana pun
  onUpdateCriterion,
}) => {
  ...
  // Helper to ensure criterion has checklist initialized
  ...
  // LOGIKA FILTERING (Pencarian & Status & Domain)
  ...
  // Categorize items BERDASARKAN HASIL FILTER
  ...
  // Sort each list: Foundational domain gap > 0 items pinned at top, then Priority Score desc
    ...
    return (
      <div
        ...
        className={`p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-3 transition-all ${
          isFoundational
            ? 'ring-1 ring-amber-400 border-l-4 border-l-amber-500'
            : score >= 61
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-200 uppercase tracking-wider border border-slate-200 dark:border-slate-700">
              {item.domain} • {item.stage}
            </span>
            {isFoundational && (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-200 border border-amber-300 dark:border-amber-800">
                ⚠️ Fondasional
              </span>
            )}
          </div>
          <span className="text-[10px] font-mono font-extrabold text-slate-600 dark:text-slate-400">
            Priority Score: {score}%
          </span>
        </div>

        {isFoundational && foundationalJustification && (
          <div className="p-2.5 rounded-xl bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 text-[11px] text-amber-900 dark:text-amber-200 font-medium">
        ...
        {/* NEW: Saran Tindakan Konkret Box */}
        <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 space-y-1">
          <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-slate-800 dark:text-slate-200">
        ...
        <div className="p-3 rounded-xl bg-slate-50/60 dark:bg-slate-800/40 border border-slate-200/80 dark:border-slate-700/60 space-y-3 text-xs">
          {/* PIC & Deadline & Status */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">PIC Eksekutor</label>
              <input
                type="text"
                value={item.actionPic || item.pic || ''}
                onChange={(e) => onUpdateCriterion(item.id, { actionPic: e.target.value })}
                placeholder="Team IT Security"
                className="w-full px-2 py-1 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-[11px]"
              />
            </div>

            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">Target Deadline</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-0.5">Status Remediasi</label>
              <select
                value={item.actionStatus || 'Not Started'}
                onChange={(e) => onUpdateCriterion(item.id, { actionStatus: e.target.value as ActionStatus })}
                className="w-full px-2 py-1 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-[11px] font-medium"
              >
          ...
          <div className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
            ...
            <div className="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
              <div
                className="bg-slate-900 dark:bg-indigo-400 h-full transition-all duration-300"
            ...
            <div className="space-y-1.5 pt-1">
              {checklist.map((m) => (
                <label
                  key={m.id}
                  className={`flex items-center gap-2 p-1.5 rounded-lg text-[11px] transition-colors select-none cursor-pointer ${
                    m.completed
                      ? 'bg-emerald-50/70 dark:bg-emerald-950/30 text-emerald-950 dark:text-emerald-200 font-medium'
                ...
                >
                  ...
                  />
                  <span className={m.completed ? 'line-through opacity-80' : ''}>{m.label}</span>
                </label>
              ))}
            </div>
            ...
            <button
              ...
              className="mt-1 flex items-center gap-1 text-[10px] font-bold text-slate-700 dark:text-slate-300 hover:underline"
            >
          ...
          <div>
            <input
              ...
              placeholder="Catatan tindak lanjut / URL dokumen bukti..."
              className="w-full px-2 py-1 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-[11px]"
            />
          </div>
        </div>
